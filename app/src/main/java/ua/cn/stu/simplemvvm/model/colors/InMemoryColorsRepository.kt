...
class InMemoryColorsRepository(
    ...
    private val currentColorFlow = MutableSharedFlow<NamedColor>(
        // Replay не используем потому что у нас уже есть отдельный метод getCurrentColor
        replay = 0,
        extraBufferCapacity = 1, // Потому что мы используем параметр BufferOverFlow
    ...
    override suspend fun getById(id: Long): NamedColor = withContext(ioDispatcher.value) {
        delay(100) // Тут регулировка задержки получения цвета.
